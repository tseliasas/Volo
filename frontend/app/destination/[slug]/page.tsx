...
export default function DestinationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#07111A] text-white font-sans relative">
      ...
      <div className="flex flex-col gap-5 border-b border-white/10 pb-6 mb-8 w-full relative z-10 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-emerald-400/80 font-bold uppercase tracking-[0.3em] mt-2 ml-1">
            {country}
          </p>
        </div>
      ...
      <div className="flex flex-col lg:flex-row items-start gap-10 w-full relative z-10">
        ...
        <div className="flex-1 w-full border border-white/10 bg-[#0B1520] rounded-[28px] p-5 sm:rounded-[38px] sm:p-10">
          ...
          {!loading && itinerary.length > 0 && (
            <div className="relative border-l border-emerald-400/30 ml-4 space-y-10 py-4">
              {(() => {
                ...
                return itinerary.map((dayPlan, index) => {
                  ...
                  return (
                    <div key={index} className="relative pl-10">
                      <div className="absolute -left-[17px] top-1 h-8 w-8 rounded-full bg-[#07111A] border-2 border-emerald-400 flex items-center justify-center">
                      ...
                      <div className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-colors">
                        <div className="flex flex-col gap-3 mb-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <span className="text-emerald-400 font-bold text-sm uppercase tracking-wider mb-1 block">{tran.day} {dayPlan.day}</span>
        ...
        <div className="w-full lg:w-[420px] shrink-0 sticky top-10 flex flex-col gap-6">
            <div className="bg-[#0B1520] border border-white/10 rounded-[28px] overflow-hidden sm:rounded-[32px]">
                    ...
                    <div className="group relative rounded-2xl bg-white/5 border border-white/10 p-4 hover:bg-white/10 transition-all overflow-hidden">
                        <div className="flex items-start gap-4 relative z-10">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="font-semibold text-white">{displayOrigin} <span className="text-emerald-400">→</span> {city}</p>
                            </div>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-[#07111A]/90 flex items-center justify-center p-6 z-[100] animate-in fade-in duration-200">
          <div className="max-w-sm w-full bg-[#0B1520] border border-white/10 rounded-[38px] p-8 relative">
            ...
            <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
              <div>
                ...
                <input type="text" value={authUsername} onChange={(e) => setAuthUsername(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:border-emerald-400 focus:outline-none transition-colors" required />
              </div>
              <div>
                <label className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-2 block">Password</label>
                <input type="password" value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:border-emerald-400 focus:outline-none transition-colors" required />
              </div>
              ...
              <button type="submit" disabled={modalLoading} className="mt-4 flex items-center justify-center gap-3 w-full py-4 rounded-xl bg-emerald-500 hover:bg-emerald-400 disabled:bg-gray-600 text-black font-bold text-lg transition-colors">
